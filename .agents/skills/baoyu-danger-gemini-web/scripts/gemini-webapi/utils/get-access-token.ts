...
function list_cached_1psidts(
  dir: string,
): Array<{ sid: string; sidts: string }> {
...
async function fetch_google_extra_cookies(
  ...
  void proxy;
  try {
    const res = await fetch_with_timeout(Endpoint.GOOGLE, {
      timeout_ms: 15_000,
    });
    ...
    if (verbose)
      logger.debug(
        `Skipping google.com preflight: ${e instanceof Error ? e.message : String(e)}`,
      );
  }
  return {};
}
...
export async function get_access_token(
  ...
  if (base_cookies["__Secure-1PSID"] && !base_cookies["__Secure-1PSIDTS"]) {
    ...
    if (sidts) {
      candidates.push(
        merge_cookie_maps(extra, base_cookies, { "__Secure-1PSIDTS": sidts }),
      );
    } else if (verbose) {
      logger.debug(
        "Skipping loading cached cookies. Cache file not found or empty.",
      );
    }
    ...
    for (const c of caches) {
      candidates.push(
        merge_cookie_maps(extra, {
          "__Secure-1PSID": c.sid,
          "__Secure-1PSIDTS": c.sidts,
        }),
      );
    }
  ...
  const try_candidates = async (): Promise<
  ...
  > => {
    ...
    const attempts = unique.map(async (c, i) => {
      try {
        if (verbose)
          logger.debug(`Init attempt (${i + 1}/${unique.length})...`);
  ...
  try {
    ...
    if (verbose)
      logger.debug(
        "Cookie attempts failed. Falling back to Chrome CDP cookie load...",
      );
  }
