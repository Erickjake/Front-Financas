...
export abstract class GemMixin {
  ...
  protected abstract _batch_execute(
    payloads: RPCData[],
    opts?: RequestInit,
  ): Promise<Response>;
      ...
      try {
        for (const part of response_json as unknown[]) {
          ...
          if (ident === "system") {
            ...
            predefined =
              (Array.isArray(parsed) ? (parsed[2] as unknown[]) : []) ?? [];
  ...
  async create_gem(
    ...
    return await this._run(async () => {
      ...
      const res = await this._batch_execute([
        new RPCData(GRPC.CREATE_GEM, payload),
      ]);
      try {
        const response_json = extract_json_from_response(
          await res.text(),
        ) as unknown[];
        const gem_id = JSON.parse(
          String((response_json[0] as unknown[])[2]),
        )[0] as string;
