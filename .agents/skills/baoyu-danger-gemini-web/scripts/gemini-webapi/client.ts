...
export class GeminiClient extends GemMixin {
  ...
  async init(
    ...
    try {
      const [token, valid] = await get_access_token(
        this.cookies,
        this.proxy,
        vb,
      );
    ...
    return await this._run(async () => {
      ...
      const uploaded =
        ...
        (await Promise.all(
          f.map(
            async (p) =>
              [[await upload_file(p, this.proxy)], parse_file_name(p)] as [
                string[],
                string,
              ],
          ),
      ...
      try {
        const timeout_ms =
          typeof kwargs.timeout_ms === "number"
            ? kwargs.timeout_ms
            : this.timeout * 1000;
      ...
      try {
        ...
        for (
          let part_index = 0;
          part_index < response_json.length;
          part_index++
        ) {
        ...
        await this.close();
        try {
          const code = get_nested_value<number>(
            response_json,
            [0, 5, 2, 0, 1, 0],
            -1,
          );
      ...
      try {
        ...
        for (
          ...
          candidate_index < candidate_list.length;
          candidate_index++
        ) {
          ...
          if (
            /^http:\/\/googleusercontent\.com\/card_content\/\d+/.test(text)
          ) {
            text = String(get_nested_value(candidate, [22, 0], text));
          }

          const thoughts = get_nested_value<string | null>(
            candidate,
            [37, 0, 0],
            null,
          );
          ...
          if (wants_generated) {
            ...
            for (
              ...
              if (!Array.isArray(part)) continue;
              const part_body = get_nested_value<string | null>(
                part,
                [2],
                null,
              );
              ...
              try {
                const part_json = JSON.parse(part_body) as unknown[];
                const cand = get_nested_value<unknown>(
                  part_json,
                  [4, candidate_index],
                  null,
                );
                ...
                const urls = collect_strings(
                  cand,
                  (s) =>
                    s.startsWith("https://lh3.googleusercontent.com/gg-dl/"),
                  1,
                );
                if (urls.length > 0) {
                  img_body = part_json;
            ...
            const img_candidate = get_nested_value<unknown[]>(
              img_body,
              [4, candidate_index],
              [],
            );
            const finished = get_nested_value<string | null>(
              img_candidate,
              [1, 0],
              null,
            );
            ...
            const gen = get_nested_value<unknown[]>(
              img_candidate,
              [12, 7, 0],
              [],
            );
            for (let img_index = 0; img_index < gen.length; img_index++) {
              ...
              const alt =
                (typeof alt_list[img_index] === "string"
                  ...
                  : null) ??
                (typeof alt_list[0] === "string"
                  ? (alt_list[0] as string)
                  : "") ??
                "";
              generated_images.push(
                new GeneratedImage(url, title, alt, this.proxy, this.cookies),
              );
            }
  ...
  protected async _batch_execute(
    payloads: RPCData[],
    opts: RequestInit = {},
  ): Promise<Response> {
    if (!this.access_token) throw new APIError("Missing access token.");
    ...
    if (res.status !== 200) {
      await this.close();
      throw new APIError(
        `Batch execution failed with status code ${res.status}`,
      );
    }
