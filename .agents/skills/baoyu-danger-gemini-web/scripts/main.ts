...
function normalizeSessionMetadata(input: unknown): Array<string | null> {
  if (Array.isArray(input)) {
    ...
    for (const v of input.slice(0, 3))
  ...
  if (input && typeof input === "object") {
    ...
    if (Array.isArray(v1.chatMetadata))
      return normalizeSessionMetadata(v1.chatMetadata);

    const conv =
      typeof v1.conversationId === "string" ? v1.conversationId : null;
...
async function loadSession(id: string): Promise<SessionRecord | null> {
  ...
  try {
    ...
    const sid =
      ...
      (typeof (j as any).sessionId === "string" &&
        (j as any).sessionId.trim()) ||
      id;
    const metadata = normalizeSessionMetadata(
      (j as any).metadata ?? (j as any).chatMetadata ?? j,
    );
...
async function main(): Promise<void> {
  ...
  if (args.profileDir)
    process.env.GEMINI_WEB_CHROME_PROFILE_DIR = args.profileDir;
